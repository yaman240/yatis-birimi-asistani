...
const pediatricUpdateButton = document.getElementById("pediatricUpdateButton");
...
printButton.addEventListener("click", printVisibleList);
pediatricUpdateButton.addEventListener("click", updatePediatricRecords);
addUserButton.addEventListener("click", addApprovedUser);
...
async function updateAdminUi(user) {
  ...
  if (isAdmin) {
    isApprovedUser = true;
    adminStatus.textContent = `Yönetici modu • ${currentUserEmail}`;
    ...
    contentBox.classList.remove("locked-content");
    await loadApprovedUsers();
  } else if (user && currentUserEmail) {
    try {
      const snap = await getDoc(doc(db, "users", currentUserEmail));
      const userData = snap.exists() ? snap.data() : null;
      isApprovedUser = Boolean(userData?.active !== false && snap.exists());
    } catch (error) {
      console.error(error);
      isApprovedUser = false;
    }
    ...
    if (isApprovedUser) {
      adminStatus.textContent = `Kullanıcı modu • ${currentUserEmail}`;
      adminStatus.className = "admin-status user";
      loginButton.classList.add("hidden");
      logoutButton.classList.remove("hidden");
      printButton.classList.remove("hidden");
      contentBox.classList.remove("locked-content");
    } else {
      alert("Bu hesap sisteme tanımlı değil. Yöneticiye başvurun.");
      await signOut(auth);
      return;
    }
    adminPanel.classList.add("hidden");
    ...
    adminStatus.textContent = "Giriş yapılmadı • Listeyi görmek için Google ile giriş yapın.";
    adminStatus.className = "admin-status visitor";
    adminPanel.classList.add("hidden");
    newRecordButton.classList.add("hidden");
    loginButton.classList.remove("hidden");
    logoutButton.classList.add("hidden");
    printButton.classList.add("hidden");
    contentBox.classList.add("locked-content");
    clearForm();
  }
  render();
}
...
async function login() {
  ...
  try {
    ...
    console.error(error);
    if (error.code !== "auth/popup-closed-by-user") alert("Google ile giriş yapılamadı. Tekrar dene.");
  } finally {
    loginButton.disabled = false;
    loginButton.textContent = "🔐 Google ile Giriş Yap";
  }
}

async function logout() {
  try { await signOut(auth); }
  catch (error) { console.error(error); alert("Çıkış yapılamadı."); }
}

onAuthStateChanged(auth, user => { updateAdminUi(user).catch(console.error); });
...
async function addApprovedUser() {
  if (!isAdmin) return alert("Bu işlem için yönetici girişi gerekli.");
  const email = userDocId(userEmailInput.value);
  if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) return alert("Geçerli bir e-posta adresi yazın.");
  if (email === ADMIN_EMAIL) return alert("Yönetici hesabı zaten tanımlı.");
  addUserButton.disabled = true;
  try {
    await setDoc(doc(db, "users", email), { email, role: "user", active: true, updatedAt: new Date().toISOString() }, { merge: true });
    userEmailInput.value = "";
    await loadApprovedUsers();
  } catch (error) {
    console.error(error);
    alert("Kullanıcı eklenemedi. Firestore izinlerini kontrol edin.");
  } finally { addUserButton.disabled = false; }
}

async function loadApprovedUsers() {
  if (!isAdmin) return;
  userListElement.innerHTML = '<div class="muted">Kullanıcılar yükleniyor...</div>';
  try {
    const snap = await getDocs(collection(db, "users"));
    const users = snap.docs.map(d => ({ id: d.id, ...d.data() })).sort((a,b) => String(a.email||a.id).localeCompare(String(b.email||b.id), "tr"));
    if (!users.length) {
      userListElement.innerHTML = '<div class="muted">Henüz normal kullanıcı eklenmedi.</div>';
      return;
    }
    userListElement.innerHTML = users.map(u => `<div class="user-row"><div><strong>${escapeHtml(u.email || u.id)}</strong><div class="muted">${u.active === false ? "Pasif" : "Aktif"} • Sadece görüntüleme ve yazdırma</div></div><div class="user-row-actions"><button class="secondary compact-button" data-action="toggle-user" data-email="${escapeHtml(u.email || u.id)}" data-active="${u.active === false ? "false" : "true"}">${u.active === false ? "Aktif Et" : "Pasif Et"}</button><button class="danger compact-button" data-action="delete-user" data-email="${escapeHtml(u.email || u.id)}">Sil</button></div></div>`).join("");
  } catch (error) {
    console.error(error);
    userListElement.innerHTML = '<div class="muted">Kullanıcı listesi alınamadı.</div>';
  }
}
...
async function handleUserListClick(event) {
  ...
  const email = userDocId(button.dataset.email);
  if (!email) return;
  try {
    if (button.dataset.action === "delete-user") {
      ...
      await setDoc(doc(db, "users", email), { email, role: "user", active: !currentlyActive, updatedAt: new Date().toISOString() }, { merge: true });
    }
    await loadApprovedUsers();
  } catch (error) {
    console.error(error);
    alert("Kullanıcı işlemi tamamlanamadı.");
  }
}
...
function updateClinicSelect(selected = "") {
  const clinicSelect = document.getElementById("clinic");
  const clinics = [...new Set([...FIXED_CLINICS, ...data.map(item => item.clinic)])]
    .filter(Boolean).sort((a,b)=>a.localeCompare(b,"tr"));
  clinicSelect.innerHTML = '<option value="">Poliklinik seç</option>' +
    clinics.map(c => `<option value="${escapeHtml(c)}">${escapeHtml(c)}</option>`).join("");
  clinicSelect.value = clinics.includes(selected) ? selected : "";
}

function render() {
  if (!isApprovedUser) { listElement.innerHTML = `<div class="login-required"><strong>🔐 Giriş gerekli</strong><div>Fiyat listesini görmek için Google hesabınızla giriş yapın.</div></div>`; return; }
  updateFilter();
  const formClinic = document.getElementById("clinic");
  if (!formClinic.options.length) updateClinicSelect();
...
function edit(id) {
  ...
  editing = id;
  updateClinicSelect(item.clinic);
...
const PEDIATRIC_UPDATE = [{"clinic": "Çocuk Cerrahisi", "name": "Sünnet", "minPrice": 25000, "maxPrice": null, "sgkPrice": null, "privatePrice": null, "description": "Muayene ve tetkikler dahil.", "cashOnly": false}, {"clinic": "Çocuk Cerrahisi", "name": "Laparoskopik Apendektomi (Simple)", "minPrice": 60000, "maxPrice": null, "sgkPrice": null, "privatePrice": null, "description": "Tetkikler hariç.", "cashOnly": false}, {"clinic": "Çocuk Cerrahisi", "name": "Laparoskopik Perfore Apandisit", "minPrice": 100000, "maxPrice": null, "sgkPrice": null, "privatePrice": null, "description": "5 gün yatışa kadar dahil.", "cashOnly": false}, {"clinic": "Çocuk Cerrahisi", "name": "Hipospadias (Distal)", "minPrice": 75000, "maxPrice": null, "sgkPrice": null, "privatePrice": null, "description": "3 gece yatış dahil.", "cashOnly": false}, {"clinic": "Çocuk Cerrahisi", "name": "Hipospadias Proksimal - 1. Seans (Bukkal Greft)", "minPrice": 90000, "maxPrice": null, "sgkPrice": null, "privatePrice": null, "description": "3 gece yatış dahil.", "cashOnly": false}, {"clinic": "Çocuk Cerrahisi", "name": "Hipospadias Proksimal - 2. Seans", "minPrice": 100000, "maxPrice": null, "sgkPrice": null, "privatePrice": null, "description": "6 gece yatış dahil.", "cashOnly": false}, {"clinic": "Çocuk Cerrahisi", "name": "İleus Ameliyatı", "minPrice": 120000, "maxPrice": null, "sgkPrice": null, "privatePrice": null, "description": "5 gece yatış dahil.", "cashOnly": false}, {"clinic": "Çocuk Cerrahisi", "name": "Kolostomi Açılması", "minPrice": 150000, "maxPrice": null, "sgkPrice": null, "privatePrice": null, "description": "5 gece yatış dahil.", "cashOnly": false}, {"clinic": "Çocuk Cerrahisi", "name": "Kolostomi Kapatılması", "minPrice": 150000, "maxPrice": null, "sgkPrice": null, "privatePrice": null, "description": "5 gece yatış dahil.", "cashOnly": false}, {"clinic": "Çocuk Cerrahisi", "name": "Laparoskopik Kolesistektomi", "minPrice": 90000, "maxPrice": null, "sgkPrice": null, "privatePrice": null, "description": "2 gece yatış dahil.", "cashOnly": false}, {"clinic": "Çocuk Cerrahisi", "name": "Mide Perforasyonu Onarımı", "minPrice": 120000, "maxPrice": null, "sgkPrice": null, "privatePrice": null, "description": "5 gece yatış dahil.", "cashOnly": false}, {"clinic": "Çocuk Cerrahisi", "name": "Diyafragma Hernisi Onarımı", "minPrice": 150000, "maxPrice": null, "sgkPrice": null, "privatePrice": null, "description": "5 gece yatış dahil.", "cashOnly": false}, {"clinic": "Çocuk Cerrahisi", "name": "Pilor Stenozu Onarımı", "minPrice": 90000, "maxPrice": null, "sgkPrice": null, "privatePrice": null, "description": "3 gece yatış dahil.", "cashOnly": false}, {"clinic": "Çocuk Cerrahisi", "name": "Karaciğer Kist Hidatik Ameliyatı", "minPrice": 150000, "maxPrice": null, "sgkPrice": null, "privatePrice": null, "description": "4 gece yatış dahil.", "cashOnly": false}, {"clinic": "Çocuk Cerrahisi", "name": "Meckel Divertikülü Onarımı", "minPrice": 120000, "maxPrice": null, "sgkPrice": null, "privatePrice": null, "description": "4 gece yatış dahil.", "cashOnly": false}, {"clinic": "Çocuk Cerrahisi", "name": "Laparoskopik Over Kisti Eksizyonu", "minPrice": 80000, "maxPrice": null, "sgkPrice": null, "privatePrice": null, "description": "2 gece yatış dahil.", "cashOnly": false}, {"clinic": "Çocuk Cerrahisi", "name": "Laparoskopik Over Tümörü Eksizyonu (Ooforektomi ±)", "minPrice": 100000, "maxPrice": null, "sgkPrice": null, "privatePrice": null, "description": "2 gece yatış dahil.", "cashOnly": false}, {"clinic": "Çocuk Cerrahisi", "name": "İnguinal Herni Onarımı - Tek Taraf (Açık)", "minPrice": 50000, "maxPrice": null, "sgkPrice": null, "privatePrice": null, "description": "Aynı gün taburcu.", "cashOnly": false}, {"clinic": "Çocuk Cerrahisi", "name": "İnguinal Herni Onarımı - Bilateral (Açık)", "minPrice": 80000, "maxPrice": null, "sgkPrice": null, "privatePrice": null, "description": "Aynı gün taburcu.", "cashOnly": false}, {"clinic": "Çocuk Cerrahisi", "name": "Laparoskopik İnguinal Herni Onarımı - Tek Taraf", "minPrice": 60000, "maxPrice": null, "sgkPrice": null, "privatePrice": null, "description": "1 gece yatış dahil.", "cashOnly": false}, {"clinic": "Çocuk Cerrahisi", "name": "Laparoskopik İnguinal Herni Onarımı - Bilateral", "minPrice": 90000, "maxPrice": null, "sgkPrice": null, "privatePrice": null, "description": "1 gece yatış dahil.", "cashOnly": false}, {"clinic": "Çocuk Cerrahisi", "name": "Umbilikal / Epigastrik Herni Onarımı", "minPrice": 40000, "maxPrice": null, "sgkPrice": null, "privatePrice": null, "description": "Aynı gün taburcu.", "cashOnly": false}, {"clinic": "Çocuk Cerrahisi", "name": "Varikoselektomi - Tek Taraf", "minPrice": 60000, "maxPrice": null, "sgkPrice": null, "privatePrice": null, "description": "Aynı gün taburcu.", "cashOnly": false}, {"clinic": "Çocuk Cerrahisi", "name": "Spermatik Kord Kisti", "minPrice": 50000, "maxPrice": null, "sgkPrice": null, "privatePrice": null, "description": "Aynı gün taburcu.", "cashOnly": false}, {"clinic": "Çocuk Cerrahisi", "name": "Hidroselektomi - Tek Taraf", "minPrice": 50000, "maxPrice": null, "sgkPrice": null, "privatePrice": null, "description": "Aynı gün taburcu.", "cashOnly": false}, {"clinic": "Çocuk Cerrahisi", "name": "Hidroselektomi - Bilateral", "minPrice": 80000, "maxPrice": null, "sgkPrice": null, "privatePrice": null, "description": "Aynı gün taburcu.", "cashOnly": false}, {"clinic": "Çocuk Cerrahisi", "name": "Testis Torsiyonu Düzeltilmesi", "minPrice": 60000, "maxPrice": null, "sgkPrice": null, "privatePrice": null, "description": "1 gece yatış dahil.", "cashOnly": false}, {"clinic": "Çocuk Cerrahisi", "name": "Orşiektomi", "minPrice": 40000, "maxPrice": null, "sgkPrice": null, "privatePrice": null, "description": "Aynı gün taburcu.", "cashOnly": false}, {"clinic": "Çocuk Cerrahisi", "name": "İnmemiş Testis - Tek Taraf", "minPrice": 60000, "maxPrice": null, "sgkPrice": null, "privatePrice": null, "description": "Aynı gün taburcu.", "cashOnly": false}, {"clinic": "Çocuk Cerrahisi", "name": "İnmemiş Testis - Bilateral", "minPrice": 100000, "maxPrice": null, "sgkPrice": null, "privatePrice": null, "description": "1 gece yatış dahil.", "cashOnly": false}, {"clinic": "Çocuk Cerrahisi", "name": "Bartholin Kisti Eksizyonu", "minPrice": 50000, "maxPrice": null, "sgkPrice": null, "privatePrice": null, "description": "1 gece yatış dahil.", "cashOnly": false}, {"clinic": "Çocuk Cerrahisi", "name": "Pilonidal Sinüs - Flep", "minPrice": 60000, "maxPrice": null, "sgkPrice": null, "privatePrice": null, "description": "1 gece yatış dahil.", "cashOnly": false}, {"clinic": "Çocuk Cerrahisi", "name": "Pilonidal Sinüs - Lazer / Lokal", "minPrice": 20000, "maxPrice": null, "sgkPrice": null, "privatePrice": null, "description": "Aynı gün taburcu.", "cashOnly": false}, {"clinic": "Çocuk Cerrahisi", "name": "Cilt Altı Lipom Eksizyonu", "minPrice": 20000, "maxPrice": 30000, "sgkPrice": null, "privatePrice": null, "description": "Aynı gün taburcu.", "cashOnly": false}, {"clinic": "Çocuk Cerrahisi", "name": "Tırnak Yatağı Revizyonu - Tek Tırnak", "minPrice": 15000, "maxPrice": null, "sgkPrice": null, "privatePrice": null, "description": "Aynı gün taburcu.", "cashOnly": false}, {"clinic": "Çocuk Cerrahisi", "name": "Tırnak Yatağı Revizyonu - İki Tırnak", "minPrice": 25000, "maxPrice": null, "sgkPrice": null, "privatePrice": null, "description": "Aynı gün taburcu.", "cashOnly": false}, {"clinic": "Çocuk Cerrahisi", "name": "Frenulum Lingula Plastiği - Poliklinik / Lokal", "minPrice": 7500, "maxPrice": null, "sgkPrice": null, "privatePrice": null, "description": "Sanal yatış.", "cashOnly": false}, {"clinic": "Çocuk Cerrahisi", "name": "Hipertrofik Dil Bağı Eksizyonu - Ameliyathane", "minPrice": 15000, "maxPrice": null, "sgkPrice": null, "privatePrice": null, "description": "Aynı gün taburcu.", "cashOnly": false}, {"clinic": "Çocuk Cerrahisi", "name": "Labial Füzyon Açılması - Poliklinik", "minPrice": 4000, "maxPrice": null, "sgkPrice": null, "privatePrice": null, "description": "Sanal yatış.", "cashOnly": false}, {"clinic": "Çocuk Cerrahisi", "name": "Lenf Nodu Eksizyonel Biyopsisi", "minPrice": 30000, "maxPrice": null, "sgkPrice": null, "privatePrice": null, "description": "Aynı gün taburcu.", "cashOnly": false}, {"clinic": "Çocuk Cerrahisi", "name": "Pnömotoraks - Tüp Torakostomi", "minPrice": 50000, "maxPrice": null, "sgkPrice": null, "privatePrice": null, "description": "Yatış süresi hariç.", "cashOnly": false}, {"clinic": "Çocuk Cerrahisi", "name": "Akciğer Bül / Blep Eksizyonu - Torakoskopik", "minPrice": 120000, "maxPrice": null, "sgkPrice": null, "privatePrice": null, "description": "5 gece yatış dahil.", "cashOnly": false}, {"clinic": "Çocuk Cerrahisi", "name": "Memeden Kist / Kitle Çıkarılması", "minPrice": 60000, "maxPrice": null, "sgkPrice": null, "privatePrice": null, "description": "Aynı gün taburcu.", "cashOnly": false}, {"clinic": "Çocuk Cerrahisi", "name": "Jinekomasti Düzeltilmesi - Tek Taraf", "minPrice": 60000, "maxPrice": null, "sgkPrice": null, "privatePrice": null, "description": "1 gece yatış dahil.", "cashOnly": false}, {"clinic": "Çocuk Cerrahisi", "name": "Jinekomasti Düzeltilmesi - Bilateral", "minPrice": 100000, "maxPrice": null, "sgkPrice": null, "privatePrice": null, "description": "1 gece yatış dahil.", "cashOnly": false}, {"clinic": "Çocuk Cerrahisi", "name": "Pektus Ekskavatum Düzeltilmesi - Nuss Cerrahisi", "minPrice": 200000, "maxPrice": null, "sgkPrice": null, "privatePrice": null, "description": "5 gece yatış dahil.", "cashOnly": false}, {"clinic": "Çocuk Cerrahisi", "name": "Kesi Süturasyonu - 5 cm Altı", "minPrice": 20000, "maxPrice": null, "sgkPrice": null, "privatePrice": null, "description": "Aynı gün taburcu.", "cashOnly": false}, {"clinic": "Çocuk Cerrahisi", "name": "Kesi Süturasyonu - 5 cm Üstü", "minPrice": 30000, "maxPrice": 40000, "sgkPrice": null, "privatePrice": null, "description": "1 gece yatış dahil.", "cashOnly": false}, {"clinic": "Çocuk Cerrahisi", "name": "Lenfanjiom - Bleomisin Enjeksiyonu", "minPrice": 25000, "maxPrice": null, "sgkPrice": null, "privatePrice": null, "description": "İlaç hariç.", "cashOnly": false}, {"clinic": "Çocuk Cerrahisi", "name": "Lenfanjiom - Açık Cerrahi", "minPrice": 80000, "maxPrice": 100000, "sgkPrice": null, "privatePrice": null, "description": "1 gece yatış dahil.", "cashOnly": false}, {"clinic": "Çocuk Cerrahisi", "name": "Laparoskopik Urakus Kisti Eksizyonu", "minPrice": 100000, "maxPrice": null, "sgkPrice": null, "privatePrice": null, "description": "4 gece yatış dahil.", "cashOnly": false}, {"clinic": "Çocuk Cerrahisi", "name": "Fimozis Açılması - Poliklinik", "minPrice": 5000, "maxPrice": null, "sgkPrice": null, "privatePrice": null, "description": "Sanal yatış.", "cashOnly": false}];

function visibleRows() {
  const query = document.getElementById("search").value.toLocaleLowerCase("tr-TR");
  const selectedClinic = document.getElementById("filter").value;
  return data.filter(item => {
    const text = `${item.name} ${item.clinic} ${item.description}`.toLocaleLowerCase("tr-TR");
    return (!selectedClinic || item.clinic === selectedClinic) && text.includes(query);
  });
}
...
function printVisibleList() {
  ...
  if (!rows.length) return alert("Yazdırılacak kayıt bulunamadı.");
  const title = document.getElementById("filter").value || "Tüm Branşlar";
  const win = window.open("", "_blank", "width=1000,height=750");
  if (!win) return alert("Yazdırma penceresi engellendi. Açılır pencereye izin ver.");
  const body = rows.map(item => `<tr><td>${escapeHtml(item.clinic)}</td><td>${escapeHtml(item.name)}</td><td>${prices(item).replaceAll('class="chip"','')}</td><td>${escapeHtml(item.description || "")}</td></tr>`).join("");
  win.document.write(`<!doctype html><html lang="tr"><head><meta charset="utf-8"><title>${escapeHtml(title)} Fiyat Listesi</title><style>@page{size:A4 portrait;margin:12mm}body{font-family:Arial,sans-serif;color:#111;font-size:11px}h1{font-size:20px;margin:0 0 4px}.date{margin-bottom:14px;color:#555}table{width:100%;border-collapse:collapse}th,td{border:1px solid #bbb;padding:6px;vertical-align:top}th{background:#eee;text-align:left}tr{break-inside:avoid}</style></head><body><h1>${escapeHtml(title)} Fiyat Listesi</h1><div class="date">Tarih: ${new Date().toLocaleDateString("tr-TR")}</div><table><thead><tr><th>Branş</th><th>İşlem</th><th>Fiyat</th><th>Açıklama</th></tr></thead><tbody>${body}</tbody></table><script>window.onload=()=>{window.print()}<\/script></body></html>`);
  win.document.close();
}

async function updatePediatricRecords() {
  if (!isAdmin) return alert("Bu işlem için yönetici girişi gerekli.");
  if (!confirm(`Çocuk Cerrahisi listesindeki ${PEDIATRIC_UPDATE.length} kayıt eklensin veya güncellensin mi?`)) return;
  pediatricUpdateButton.disabled = true;
  pediatricUpdateButton.textContent = "Güncelleniyor...";
  try {
    const existingByName = new Map(data.filter(x => x.clinic === "Çocuk Cerrahisi").map(x => [x.name.toLocaleLowerCase("tr-TR"), x]));
    const batch = writeBatch(db);
    PEDIATRIC_UPDATE.forEach((record, index) => {
      const existing = existingByName.get(record.name.toLocaleLowerCase("tr-TR"));
      const item = { ...record, id: existing?.id || (470000 + index) };
      batch.set(doc(db, COLLECTION_NAME, String(item.id)), item);
    });
    await batch.commit();
    alert("Çocuk Cerrahisi listesi güncellendi.");
  } catch (error) {
    console.error(error);
    alert("Çocuk Cerrahisi listesi güncellenemedi.");
  } finally {
    pediatricUpdateButton.disabled = false;
    pediatricUpdateButton.textContent = "Çocuk Cerrahisi listesini güncelle";
  }
}
