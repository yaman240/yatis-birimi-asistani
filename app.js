...
userManagementButton?.addEventListener("click", () => {
  ...
  userManagementPanel.classList.toggle("hidden");
});
...
function startUserManagement() {
  if (stopUserSubscription || !currentPermissions.viewUserManagement) return;
    ...
    <div class="user-management-row" data-user-uid="${escapeHtml(user.uid || user.id)}" data-user-display-name="${escapeHtml(user.displayName || "")}">
      ...
      <div class="user-management-controls">
        <select data-user-role aria-label="Kullanıcı rolü">
          ${["personel", "doktor", "idari", "admin"].map(role => `<option value="${role}" ${user.role === role ? "selected" : ""}>${role}</option>`).join("")}
        </select>
...
onAuthStateChanged(auth, async user => {
  ...
  if (!user) {
    ...
    setConnectionStatus("Fiyat listesini görüntülemek için Google ile giriş yapın.");
    listElement.innerHTML = '<p class="muted">Fiyatları görüntülemek için giriş yapın.</p>';
    return;
  }
  try {
    const profile = await userProfiles.getByUid(user.uid);
    if (revision !== authStateRevision) return;
    updateAdminUi(user, profile);
    if (currentPermissions.viewPrices) {
      priceListener.start();
    } else {
      setConnectionStatus("Fiyatlara erişmek için aktif bir kullanıcı rolü gerekli.");
      listElement.innerHTML = '<p class="muted">Bu hesap fiyat listesini görüntüleme yetkisine sahip değil.</p>';
    }
  } catch (error) {
    if (revision !== authStateRevision) return;
    console.error(error);
